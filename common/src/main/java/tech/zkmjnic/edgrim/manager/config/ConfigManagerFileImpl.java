...
public class ConfigManagerFileImpl implements ConfigManager, BasicReloadable {
    ...
    private File getConfigFile(String path) {
        return new File(EdGrimAPI.INSTANCE.getGrimPlugin().getDataFolder(), path);
    }

    @Override
    public void reload() {
        EdGrimAPI.INSTANCE.getGrimPlugin().getDataFolder().mkdirs();
        if (!initialized) {
            initialized = true;
            config.addSource(EdGrimAPI.class, "config", getConfigFile("config.yml"));
            config.addSource(EdGrimAPI.class, "messages", getConfigFile("messages.yml"));
            config.addSource(EdGrimAPI.class, "discord", getConfigFile("discord.yml"));
            config.addSource(EdGrimAPI.class, "punishments", getConfigFile("punishments.yml"));
        }
        //
    ...
    private void upgrade() {
        File config = new File(EdGrimAPI.INSTANCE.getGrimPlugin().getDataFolder(), "config.yml");
        if (config.exists()) {
            try {
    ...
    private void removeLegacyTwoPointOne(File config) throws IOException {
        ...
        Files.move(config.toPath(), new File(EdGrimAPI.INSTANCE.getGrimPlugin().getDataFolder(), "config-2.1.old.yml").toPath());
    }
    ...
    private void addMissingPunishments() {
        File config = new File(EdGrimAPI.INSTANCE.getGrimPlugin().getDataFolder(), "punishments.yml");
        String configString;
        if (config.exists()) {
            try {
    ...
    private void fixBadPacketsAndAdjustPingConfig(File config, String configString) {
        ...
        File punishConfig = new File(EdGrimAPI.INSTANCE.getGrimPlugin().getDataFolder(), "punishments.yml");
    ...
    private void newOffsetNewDiscordConf(File config, String configString) throws IOException {
        ...
        File discordFile = new File(EdGrimAPI.INSTANCE.getGrimPlugin().getDataFolder(), "discord.yml");
    ...
    private void addSuperDebug(File config, String configString) throws IOException {
        ...
        File messageFile = new File(EdGrimAPI.INSTANCE.getGrimPlugin().getDataFolder(), "messages.yml");
