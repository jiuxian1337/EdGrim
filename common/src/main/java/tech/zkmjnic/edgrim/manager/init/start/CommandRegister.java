...
public class CommandRegister implements StartableInitable {
    ...
    public void start() {
        ...
        if (EdGrimAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("check-for-updates", true)) {
            GrimVersion.checkForUpdatesAsync(EdGrimAPI.INSTANCE.getPlatformServer().getConsoleSender());
        }
    }
}
