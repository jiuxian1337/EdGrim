...
public class GrimVersion implements BuildableCommand {
    ...
    public static void checkForUpdatesAsync(Sender sender) {
        ...
        lastCheck = now;
        EdGrimAPI.INSTANCE.getScheduler().getAsyncScheduler().runNow(EdGrimAPI.INSTANCE.getGrimPlugin(), () -> checkForUpdates(sender));
    }
    ...
    private static void checkForUpdates(Sender sender) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    ...
                    .GET()
                    .header("User-Agent", "EdGrim/" + EdGrimAPI.INSTANCE.getExternalAPI().getGrimVersion())
            ...
            if (statusCode != 200) {
                ...
                sender.sendMessage(Objects.requireNonNullElseGet(msg, () -> Component.text()
                        ...
                        .append(Component.text(" Failed to check latest EdGrim version. Update server responded with code: ")
            ...
            if (object.has("status")) {
                ...
                status = Status.SemVer.getVersionStatus(EdGrimAPI.INSTANCE.getExternalAPI().getGrimVersion(), latest);
            }
            //
            Component msg = switch (status) {
                case AHEAD ->
                        Component.text("You are using a development version of EdGrim").color(NamedTextColor.LIGHT_PURPLE);
                case UPDATED ->
                        Component.text("You are using the latest version of EdGrim").color(NamedTextColor.GREEN);
                case OUTDATED -> Component.text()
                        .append(Component.text("New EdGrim version found!").color(NamedTextColor.AQUA))
                        ...
                        .build();
                case UNKNOWN ->
                        Component.text("You are using an unknown EdGrim version.").color(NamedTextColor.RED);
            };
            ...
            LogUtil.error("Failed to check latest EdGrim version.", e);
        }
    }
