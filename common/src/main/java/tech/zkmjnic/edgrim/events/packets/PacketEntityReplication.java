...
import tech.zkmjnic.edgrim.utils.data.packetentity.PacketEntityHook;
import tech.zkmjnic.edgrim.utils.data.packetentity.PacketEntityTrackXRot;
            ...
            if (slot.getWindowId() == 0) {
                Runnable task = () -> {
                    if (slot.getSlot() - 36 == player.packetStateData.lastSlotSelected && (
                        ...
                        if (player.isResetItemUsageOnItemUpdate() && hand == EdGrimAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer)) {
                            EdGrimAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                        }
                    }
                };
                ...
                Runnable task = () -> {
                    if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_8)) {
                        ...
                        if (items.getItems().size() > 45 && !player.inventory.getOffHand().is(items.getItems().get(45).getType())) {
                            ...
                            if (player.isResetItemUsageOnItemUpdate() && EdGrimAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.OFF_HAND) {
                                EdGrimAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                            }
                        }
                        ...
                        if (!player.inventory.getHeldItem().is(items.getItems().get(player.packetStateData.lastSlotSelected + 36).getType())) {
                            ...
                            if (player.isResetItemUsageOnItemUpdate() && EdGrimAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.MAIN_HAND) {
                                EdGrimAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                            }
                        }
                    }
