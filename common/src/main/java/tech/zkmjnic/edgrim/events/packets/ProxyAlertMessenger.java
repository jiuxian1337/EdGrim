...
public class ProxyAlertMessenger extends PacketListenerAbstract {
    ...
    public ProxyAlertMessenger() {
        ...
        if (usingProxy) {
            ...
            EdGrimAPI.INSTANCE.getPlatformServer().registerOutgoingPluginChannel("BungeeCord");
        }
    }
    ...
    public static void sendPluginMessage(String message) {
        ...
        Iterables.getFirst(EdGrimAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers(), null).sendPluginMessage("BungeeCord", out.toByteArray());
    }

    public static boolean canSendAlerts() {
        return usingProxy && EdGrimAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("alerts.proxy.send", false) && !EdGrimAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers().isEmpty();
    }

    public static boolean canReceiveAlerts() {
        return usingProxy && EdGrimAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("alerts.proxy.receive", false) && EdGrimAPI.INSTANCE.getAlertManager().hasAlertListeners();
    }
    ...
    public void onPacketReceive(final PacketReceiveEvent event) {
        ...
        if (!in.readUTF().equals("EdGrim")) return;
