...
public class ClientBrand extends Check implements PacketCheck {
    ...
    private void handle(String channel, byte[] data) {
        ...
        if (data.length > 64 || data.length == 0) {
            ...
            if (!EdGrimAPI.INSTANCE.getConfigManager().isIgnoredClient(brand)) {
                String message = EdGrimAPI.INSTANCE.getConfigManager().getConfig().getStringElse("client-brand-format", "%prefix% &f%player% joined using %brand%");
                Component component = MessageUtil.replacePlaceholders(player, MessageUtil.miniMessage(message));

                EdGrimAPI.INSTANCE.getAlertManager().sendBrand(component, null);
            }
        }
        ...
        if (hasReachHacks && EdGrimAPI.INSTANCE.getConfigManager().isBlockBlacklistedForgeClients()) {
            player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, EdGrimAPI.INSTANCE.getConfigManager().getDisconnectBlacklistedForge())));
        }
