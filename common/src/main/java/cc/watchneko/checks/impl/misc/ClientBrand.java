...
public class ClientBrand extends Check implements PacketCheck {
    ...
    private void handle(String channel, byte[] data) {
        ...
        if (data.length > 64 || data.length == 0) {
            ...
            if (!WatchNekoAPI.INSTANCE.getConfigManager().isIgnoredClient(brand)) {
                String message = WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getStringElse("client-brand-format", "%prefix% &f%player% joined using %brand%");
                Component component = MessageUtil.replacePlaceholders(player, MessageUtil.miniMessage(message));

                WatchNekoAPI.INSTANCE.getAlertManager().sendBrand(component, null);
            }
        }
        ...
        if (hasReachHacks && WatchNekoAPI.INSTANCE.getConfigManager().isBlockBlacklistedForgeClients()) {
            player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, WatchNekoAPI.INSTANCE.getConfigManager().getDisconnectBlacklistedForge())));
        }
