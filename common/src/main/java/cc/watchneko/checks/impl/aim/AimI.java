...
import cc.watchneko.utils.lists.Tuple;
import cc.watchneko.utils.math.MathUtil;
import cc.watchneko.utils.math.Vec2d;
...
public final class AimI extends Check implements RotationCheck {
    ...
    public AimI(PlayerData player) {
        ...
        kireikoGeneric = new CopyOnWriteArrayList<>();
        historySpikes = new ArrayList<>(5);
    }
    ...
    private void checkRaw(RotationUpdate update) {
        ...
        final int distinctX = MathUtil.getDistinct(xList);
        final double shannonYaw = MathUtil.getShannonEntropy(xList);
        final double shannonPitch = MathUtil.getShannonEntropy(yList);
        final boolean invalidSensitivity = sens < 50;
        final boolean validSensitivity = sens >= 60 && sens <= 150 && sensTemp >= 60 && sensTemp < 150;
        final double deltaYaw = Math.abs(shannonYaw - oldShannonYaw);
        final double deltaPitch = Math.abs(shannonPitch - oldShannonPitch);
        ...
        if (deltaYaw < 0.005 && shannonPitch < 0.1 && player.onGround) {
            ...
            if (bufferChange > 6 && flagAndAlert("(Change)\nsy= " + String.format("%.3f", shannonYaw))) {
        ...
        if (!invalidSensitivity) {
            ...
            if ((shannonYaw - oldShannonYaw) * (shannonPitch - oldShannonPitch) < -0.18
                    && Math.abs(shannonYaw - oldShannonYaw) > 0.25
                    && Math.abs(shannonPitch - oldShannonPitch) > 0.25) {
                bufferVariation = modifyBuffer(bufferVariation, (shannonYaw > 2.9 || shannonPitch > 2.8) ? 1.4 : 1.0);
                if (bufferVariation > 6.5 && flagAndAlert(String.format("(Backward)\ny= %.3f\np= %.3f\nsy= %.3f\nsp= %.3f", deltaYaw, deltaPitch, shannonYaw, shannonPitch))) {
                    bufferVariation *= 0.7;
                    player.mitigateDamage();
                }
            }
        ...
        if (distinctX < 8 && Math.abs(MathUtil.getAverage(xList)) > 2.5) {
            ...
            if (bufferDistinct >= 4.0 && flagAndAlert("(Distinct)\ndistinct= " + distinctX + "\navg=" + MathUtil.getAverage(xList))) {
                if (bufferDistinct >= 6) player.mitigateDamage();
    ...
    private void checkSpikes() {
        ...
        Vec2d kireikoVec = new Vec2d(MathUtil.getKireikoGeneric(gcdYaw), MathUtil.getKireikoGeneric(gcdPitch));
        ...
        if (gcdYaw.size() > 8) {
            ...
            double microEntropy = MathUtil.getMicroChangeEntropy(last8);
        ...
        if (gcdYaw.size() > 15 && stabilityCheckCounter++ % 3 == 0) {
            double stability = MathUtil.getStabilityIndex(gcdYaw);
        ...
        if (!yawOutY.isEmpty() && yawOutY.size() < 4) {
            ...
            if (gcdYaw.size() > 10) {
                double rollingStd = MathUtil.getRollingStdDev(gcdYaw, 5);
            ...
            if (player.checkManager.getRotationCheck(AimProcessor.class).getDeltaYaw() < 25) {
                double maxAbs = Math.max(Math.abs(MathUtil.getMax(yawOutY)), Math.abs(MathUtil.getMin(yawOutY)));
    ...
    private boolean isInCombat() {
        return player.actionManager.hasAttackedSince(800L);
    }
