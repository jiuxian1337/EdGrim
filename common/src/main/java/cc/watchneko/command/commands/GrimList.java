...
import cc.watchneko.command.CommandUtils;
...
public class GrimList implements BuildableCommand {
    ...
    public void register(CommandManager<Sender> commandManager) {
        commandManager.command(commandManager.commandBuilder("watchneko")
                .literal("list")
                .permission("watchneko.list")
    ...
    private void handleListPlayers(Sender sender) {
        ...
        final Map<UUID, PlatformPlayer> onlinePlayers = WatchNekoAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers()
        ...
        //
        for (PlayerData entry : WatchNekoAPI.INSTANCE.getPlayerDataManager().getEntries()) {
            if (after) {
            ...
            boolean exempt = !WatchNekoAPI.INSTANCE.getPlayerDataManager().shouldCheck(entry.user);
