...
import cc.watchneko.checks.impl.packetorder.PacketOrderProcessor;
import cc.watchneko.events.packets.CheckManagerListener;
import cc.watchneko.manager.*;
import cc.watchneko.manager.player.features.FeatureManagerImpl;
import cc.watchneko.manager.player.handlers.DefaultResyncHandler;
import cc.watchneko.platform.api.player.PlatformPlayer;
import cc.watchneko.predictionengine.MovementCheckRunner;
import cc.watchneko.predictionengine.PointThreeEstimator;
import cc.watchneko.predictionengine.UncertaintyHandler;
...
import cc.watchneko.utils.change.PlayerBlockHistory;
import cc.watchneko.utils.collisions.datatypes.SimpleCollisionBox;
import cc.watchneko.utils.data.*;
import cc.watchneko.utils.data.packetentity.PacketEntity;
import cc.watchneko.utils.data.packetentity.PacketEntityHappyGhast;
import cc.watchneko.utils.data.packetentity.PacketEntitySelf;
import cc.watchneko.utils.data.tags.SyncedTags;
import cc.watchneko.utils.enums.FluidTag;
import cc.watchneko.utils.enums.Pose;
import cc.watchneko.utils.latency.*;
import cc.watchneko.utils.lists.EvictingList;
import cc.watchneko.utils.math.*;
import cc.watchneko.utils.nmsutil.BlockProperties;
import cc.watchneko.utils.nmsutil.Collisions;
import cc.watchneko.utils.nmsutil.GetBoundingBox;
import cc.watchneko.utils.nmsutil.Materials;
...
public class PlayerData implements GrimUser {
    ...
    @Getter
    private long mitigateDamageTime;
    ...
    public void onPacketCancel() {
        if (spamThreshold != -1 && cancelledPackets.incrementAndGet() > spamThreshold) {
            ...
            disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, WatchNekoAPI.INSTANCE.getConfigManager().getDisconnectClosed())));
    ...
    public void timedOut() {
        disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, WatchNekoAPI.INSTANCE.getConfigManager().getDisconnectTimeout())));
    }
    ...
    public void disconnect(Component reason) {
        ...
        try {
            user.sendPacket(new WrapperPlayServerDisconnect(reason));
                ...
                Exception ignored) { // There may (?) be an exception if the player is in the wrong state...
            LogUtil.warn("Failed to send disconnect packet to disconnect " + user.getProfile().getName() + "! Disconnecting anyways.");
        }
        user.closeConnection();
        if (platformPlayer != null) {
            WatchNekoAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, WatchNekoAPI.INSTANCE.getGrimPlugin(),
    ...
    public void pollData() {
        ...
        if (!WatchNekoAPI.INSTANCE.getPlayerDataManager().shouldCheck(user)) {
            WatchNekoAPI.INSTANCE.getPlayerDataManager().remove(user);
        }
        ...
        if (uuid != null && this.platformPlayer == null) {
            this.platformPlayer = WatchNekoAPI.INSTANCE.getPlatformPlayerFactory().getFromUUID(uuid);
    ...
    public void updateVelocityMovementSkipping() {
        ...
        if (firstBreadKB != null)
            knockback.add(new VectorData(firstBreadKB.vector, VectorData.VectorType.Knockback));
        if (likelyKB != null)
            knockback.add(new VectorData(likelyKB.vector, VectorData.VectorType.Knockback));
    ...
    public void updatePermissions() {
        ...
        try {
            WatchNekoAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, WatchNekoAPI.INSTANCE.getGrimPlugin(), () -> {
                this.noModifyPacketPermission = platformPlayer.hasPermission("watchneko.nomodifypacket");
                this.noSetbackPermission = platformPlayer.hasPermission("watchneko.nosetback");
