...
public class ProxyAlertMessenger extends PacketListenerAbstract {
    ...
    public ProxyAlertMessenger() {
        ...
        if (usingProxy) {
            ...
            WatchNekoAPI.INSTANCE.getPlatformServer().registerOutgoingPluginChannel("BungeeCord");
        }
    }
    ...
    public static void sendPluginMessage(String message) {
        ...
        Iterables.getFirst(WatchNekoAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers(), null).sendPluginMessage("BungeeCord", out.toByteArray());
    }

    public static boolean canSendAlerts() {
        return usingProxy && WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("alerts.proxy.send", false) && !WatchNekoAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers().isEmpty();
    }

    public static boolean canReceiveAlerts() {
        return usingProxy && WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("alerts.proxy.receive", false) && WatchNekoAPI.INSTANCE.getAlertManager().hasAlertListeners();
    }
    ...
    public void onPacketReceive(final PacketReceiveEvent event) {
        ...
        if (!in.readUTF().equals("WatchNeko")) return;
