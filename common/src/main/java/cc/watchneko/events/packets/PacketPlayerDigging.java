...
import cc.watchneko.checks.impl.movement.NoSlowA;
import cc.watchneko.player.PlayerData;
import cc.watchneko.utils.item.ItemBehaviour;
import cc.watchneko.utils.item.ItemBehaviourRegistry;
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        if (event.getPacketType() == PacketType.Play.Client.PLAYER_DIGGING) {
            ...
            if (dig.getAction() == DiggingAction.RELEASE_USE_ITEM) {
                final PlayerData player = WatchNekoAPI.INSTANCE.getPlayerDataManager().getPlayer(event.getUser());
        ...
        if (event.getPacketType() == PacketType.Play.Client.HELD_ITEM_CHANGE) {
            ...
            if (player.packetStateData.lastSlotSelected != slot) {
                if (player.isResetItemUsageOnSlotChange() && WatchNekoAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.MAIN_HAND) {
