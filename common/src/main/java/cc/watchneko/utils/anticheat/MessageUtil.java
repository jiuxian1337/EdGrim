package cc.watchneko.utils.anticheat;
...
public class MessageUtil {
    ...
    public @NotNull String replacePlaceholders(@Nullable PlatformPlayer player, @NotNull String string) {
        return replacePlaceholders(player == null ? null : WatchNekoAPI.INSTANCE.getPlayerDataManager().getPlayer(player.getUniqueId()), player, string, false);
    }

    private @NotNull String replacePlaceholders(@Nullable PlayerData grimPlayer, @Nullable PlatformPlayer platformPlayer, @NotNull String string, boolean removeFormatting) {
        ...
        return WatchNekoAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, string).replace(PLACEHOLDER_ESCAPE_CHAR, '%');
    }
    ...
    public @NotNull Component miniMessage(@NotNull String string) {
        string = string.replace("%prefix%", WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getStringElse("prefix", "&7[&bWatchNeko&7]"));
    ...
    public Component getParsedComponent(Sender sender, String key, String fallbackText) {
        String message = WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getStringElse(key, fallbackText);
