...
import cc.watchneko.command.commands.*;
import cc.watchneko.command.handler.GrimCommandFailureHandler;
...
public class CommandRegister implements StartableInitable {
    ...
    public void start() {
        ...
        if (WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("check-for-updates", true)) {
            GrimVersion.checkForUpdatesAsync(WatchNekoAPI.INSTANCE.getPlatformServer().getConsoleSender());
        }
    }
}
