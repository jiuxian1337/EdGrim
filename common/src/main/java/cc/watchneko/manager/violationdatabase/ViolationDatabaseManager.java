package cc.watchneko.manager.violationdatabase;
...
import cc.watchneko.manager.violationdatabase.mysql.MySQLViolationDatabase;
import cc.watchneko.manager.violationdatabase.sqlite.SQLiteViolationDatabase;
...
public class ViolationDatabaseManager implements StartableInitable, ReloadableInitable {
    ...
    public void load() {
        ConfigManager cfg = WatchNekoAPI.INSTANCE.getConfigManager().getConfig();
        ...
        switch (rawType) {
            case "SQLITE" -> {
                if (!(database instanceof SQLiteViolationDatabase)) {
                    ...
                    try {
                        ...
                        LogUtil.error(
                                """
                                        Could not load SQLite driver for /watchneko history database.
                                        Download the minecraft-sqlite-jdbc mod/plugin for SQLite support, or change history.database.type
                                        Alternatively set history.enabled=false to remove this message if /watchneko history support is not desired"""
                        );
                        this.database = NoOpViolationDatabase.INSTANCE;
                        loaded = false;
            ...
            case "MYSQL" -> {
                ...
                String db = cfg.getStringElse("history.database.database", "WatchNeko");
    ...
    public void logAlert(PlayerData player, String verbose, String checkName, int vls) {
        String grimVersion = WatchNekoAPI.INSTANCE.getExternalAPI().getGrimVersion();
        WatchNekoAPI.INSTANCE.getScheduler().getAsyncScheduler().runNow(plugin, () -> database.logAlert(player, grimVersion, verbose, checkName, vls));
    }
