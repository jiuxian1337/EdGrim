...
import cc.watchneko.manager.init.ReloadableInitable;
...
import cc.watchneko.utils.webhook.Embed;
import cc.watchneko.utils.webhook.EmbedField;
import cc.watchneko.utils.webhook.EmbedFooter;
import cc.watchneko.utils.webhook.WebhookMessage;
...
public class DiscordManager implements StartableInitable, ReloadableInitable {
    ...
    public void reload() {
        try {
            if (!WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("enabled", false)) {
                url = null;
                return;
            }

            String webhook = WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getStringElse("webhook", "");
            ...
            embedTitle = WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getStringElse("embed-title", "**WatchNeko Alert**");

            try {
                embedColor = Color.decode(WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getStringElse("embed-color", "#00FFFF")).getRGB();
            } catch (NumberFormatException e) {
                LogUtil.warn("Discord embed color is invalid");
            }

            StringBuilder sb = new StringBuilder();
            for (String string : WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getStringListElse("violation-content", getDefaultContents())) {
                sb.append(string).append("\n");
            }
            staticContent = sb.toString();
            includeTimestamp = WatchNekoAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("include-timestamp", true);
        } catch (Exception e) {
    ...
    public void sendWebhookMessage(WebhookMessage message) {
        ...
        if (!taskStarted.getAndSet(true)) {
            ...
            WatchNekoAPI.INSTANCE.getScheduler().getAsyncScheduler().runAtFixedRate(WatchNekoAPI.INSTANCE.getGrimPlugin(), DiscordManager::tick, 0, 1);
        }
    }
}
