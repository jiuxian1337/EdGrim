package cc.watchneko.manager.config;
...
public class ConfigManagerFileImpl implements ConfigManager, BasicReloadable {
    ...
    private File getConfigFile(String path) {
        return new File(WatchNekoAPI.INSTANCE.getGrimPlugin().getDataFolder(), path);
    }

    @Override
    public void reload() {
        WatchNekoAPI.INSTANCE.getGrimPlugin().getDataFolder().mkdirs();
        if (!initialized) {
            initialized = true;
            config.addSource(WatchNekoAPI.class, "config", getConfigFile("config.yml"));
            config.addSource(WatchNekoAPI.class, "messages", getConfigFile("messages.yml"));
            config.addSource(WatchNekoAPI.class, "discord", getConfigFile("discord.yml"));
            config.addSource(WatchNekoAPI.class, "punishments", getConfigFile("punishments.yml"));
        }
        //
    ...
    private void upgrade() {
        File config = new File(WatchNekoAPI.INSTANCE.getGrimPlugin().getDataFolder(), "config.yml");
        if (config.exists()) {
            try {
    ...
    private void removeLegacyTwoPointOne(File config) throws IOException {
        ...
        Files.move(config.toPath(), new File(WatchNekoAPI.INSTANCE.getGrimPlugin().getDataFolder(), "config-2.1.old.yml").toPath());
    }
    ...
    private void addMissingPunishments() {
        File config = new File(WatchNekoAPI.INSTANCE.getGrimPlugin().getDataFolder(), "punishments.yml");
        String configString;
        if (config.exists()) {
            try {
    ...
    private void fixBadPacketsAndAdjustPingConfig(File config, String configString) {
        ...
        File punishConfig = new File(WatchNekoAPI.INSTANCE.getGrimPlugin().getDataFolder(), "punishments.yml");
    ...
    private void newOffsetNewDiscordConf(File config, String configString) throws IOException {
        ...
        File discordFile = new File(WatchNekoAPI.INSTANCE.getGrimPlugin().getDataFolder(), "discord.yml");
    ...
    private void addSuperDebug(File config, String configString) throws IOException {
        ...
        File messageFile = new File(WatchNekoAPI.INSTANCE.getGrimPlugin().getDataFolder(), "messages.yml");
    ...
    private void removeAlertsOnJoin(File config, String configString) throws IOException {
        configString = configString.replaceAll("(?m)(?:  # .*\\r?\\n)?  enable-on-join: (?:true|false)\\r?\\n", "");
        Files.write(config.toPath(), configString.getBytes());
    }
