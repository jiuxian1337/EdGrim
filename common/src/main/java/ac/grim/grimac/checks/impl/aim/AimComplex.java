...
public final class AimComplex extends EdAimCheck {
    ...
    private int addGlobalVlEntropy = 0;
    private int addGlobalVlDistinct = 5;
    private int addGlobalVlRandomizer = 35;
    private int hitCancelEntropyMs = 5000;
    private int hitCancelDistinctMs = 3000;
    private int hitCancelRandomizerMs = 0;
    private int localVlLimitEntropy = 30;
    private float localVlLimitDistinct = 4.0f;
    private float localVlLimitRandomizer = 2.5f;
    ...
    public void onReload(ConfigManager config) {
        addGlobalVlEntropy = clampAddGlobalVl(config.getIntElse("AimComplex.add-global-vl-entropy", 5));
        addGlobalVlDistinct = clampAddGlobalVl(config.getIntElse("AimComplex.add-global-vl-distinct", 5));
        addGlobalVlRandomizer = clampAddGlobalVl(config.getIntElse("AimComplex.add-global-vl-randomizer", 25));
        hitCancelEntropyMs = config.getIntElse("AimComplex.hit-cancel-time-ms-entropy", 5000);
        hitCancelDistinctMs = config.getIntElse("AimComplex.hit-cancel-time-ms-distinct", 3000);
        hitCancelRandomizerMs = config.getIntElse("AimComplex.hit-cancel-time-ms-randomizer", 0);
        localVlLimitEntropy = config.getIntElse("AimComplex.local-vl-limit-entropy", 30);
        localVlLimitDistinct = (float) config.getDoubleElse("AimComplex.local-vl-limit-distinct", 4.0);
        localVlLimitRandomizer = (float) config.getDoubleElse("AimComplex.local-vl-limit-randomizer", 2.5);
    }
    ...
    private void checkRaw() {
        ...
        if (valid && difference(shannonYaw, oldShannonYaw) < 1e-5 && difference(shannonPitch, oldShannonPitch) < 1e-5) {
            entropyPerfectBuffer = Math.max(0.0f, entropyPerfectBuffer + 1.0f);
            if (entropyPerfectBuffer > localVlLimitEntropy) {
                if (addViolationsAndAlert(addGlobalVlEntropy, "t=EntropyPerfect yaw=" + shannonYaw + " pitch=" + shannonPitch + " sens=" + sens + " cs=" + clientSens)) {
                    mitigateDamage();
                }
                entropyPerfectBuffer = Math.max(0.0f, localVlLimitEntropy - 1.0f);
            }
        } else {
            entropyPerfectBuffer = 0.0f;
        }

        if (valid && difference(shannonYaw, shannonPitch) < 1e-5) {
            entropySimilarBuffer = Math.max(0.0f, entropySimilarBuffer + 1.0f);
            if (entropySimilarBuffer > localVlLimitEntropy) {
                if (addViolationsAndAlert(addGlobalVlEntropy, "t=EntropySimilar yaw=" + shannonYaw + " pitch=" + shannonPitch + " sens=" + sens + " cs=" + clientSens)) {
                    mitigateDamage();
                }
                entropySimilarBuffer = Math.max(0.0f, localVlLimitEntropy - 1.0f);
            }
        } else {
            entropySimilarBuffer = 0.0f;
        }

        if (distinctYaw < 8 && Math.abs(MathUtil.getAverage(rawYaw)) > 2.5) {
            distinctBuffer = Math.max(0.0f, distinctBuffer + 1.7f);
            if (distinctBuffer >= localVlLimitDistinct) {
                if (addViolationsAndAlert(addGlobalVlDistinct, "t=Distinct distinct=" + distinctYaw + " avg=" + MathUtil.getAverage(rawYaw) + " sens=" + sens + " cs=" + clientSens)) {
                    mitigateDamage();
                }
    ...
    private void checkSpikes() {
        ...
        if ((min < 0.09 && max > 35 && MathUtil.getMin(gcdPitch) != 0.0) && calculateSensitivity() > 50) {
            randomizerBuffer = Math.max(0.0f, randomizerBuffer + 1.0f);
            if (randomizerBuffer > localVlLimitRandomizer) {
                if (addViolationsAndAlert(addGlobalVlRandomizer, "t=RandomizerFlaw varYaw=" + devYaw + " varPitch=" + devPitch)) {
                    mitigateDamage();
                }
                randomizerBuffer = Math.max(0.0f, localVlLimitRandomizer - 1.0f);
            }
        } else {
