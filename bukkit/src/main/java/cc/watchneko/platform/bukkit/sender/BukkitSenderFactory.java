package cc.watchneko.platform.bukkit.sender;
...
public class BukkitSenderFactory extends SenderFactory<CommandSender> implements SenderMapper<CommandSender, Sender> {
    ...
    public BukkitSenderFactory() {
        this.audiences = BukkitAudiences.create(WatchNekoBukkitLoaderPlugin.LOADER);
    }
    ...
    protected void sendMessage(CommandSender sender, Component message) {
        ...
        if (sender instanceof Player || sender instanceof ConsoleCommandSender || sender instanceof RemoteConsoleCommandSender) {
            ...
            WatchNekoAPI.INSTANCE.getScheduler().getGlobalRegionScheduler().run(
                    WatchNekoAPI.INSTANCE.getGrimPlugin(),
                    () -> this.audiences.sender(sender).sendMessage(message)
            );
        }
    }
